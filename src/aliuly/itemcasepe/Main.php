...
class Main extends PluginBase implements CommandExecutor,Listener {
  ...
  public function onCommand(CommandSender $sender, Command $cmd, string $label, array $args): bool{
	...
	return $this->cmdRespawn($sender);
      case "reset":
      case "list":
	$sender->sendMessage(mc::_("Not implemented yet!"));
	return FALSE;
    }
    return FALSE;
  }
  ...
  private function cmdCancelAdd(CommandSender $c) {
    if (!Perms::inGame($c)) return TRUE;
    if (!isset($this->places[$c->getName()])) unset($this->places[$c->getName()]);
    if (!isset($this->touches[$c->getName()])) {
      $c->sendMessage(mc::_("NOT adding an ItemCase"));
      return TRUE;
    }
    unset($this->touches[$c->getName()]);
    $c->sendMessage(mc::_("Add ItemCase CANCELLED"));
    return TRUE;
  }
  private function cmdAdd(CommandSender $c) {
    if (!Perms::inGame($c)) return TRUE;
    $c->sendMessage(mc::_("Tap on the target block while holding an item"));
  ...
  public function onSendPacket(DataPacketSendEvent $ev) {
    if ($ev->getPacket()->pid() !== ProtocolInfo::FULL_CHUNK_DATA_PACKET) return;
  ...
  public function onPlayerInteract(PlayerInteractEvent $ev){
	...
	if ($bl->getID() == Block::SLAB)
	  ...
	  $pl->sendMessage(mc::_("You must place item cases on slabs"));
	  $pl->sendMessage(mc::_("or glass blocks!"));
	  return;
	}
      }
    } else {
    ...
    if ($item->getId() === Item::AIR) {
      $pl->sendMessage(mc::_("You must be holding an item!"));
      $ev->setCancelled();
      return;
    }
    ...
    if (!$this->addItemCase($bl->getLevel(),$cid,
			      ...
			      $item->getCount())) {
      $pl->sendMessage(mc::_("There is already an ItemCase there!"));
    } else {
      $pl->sendMessage(mc::_("ItemCase placed!"));
    }
  ...
  public function onBlockBreak(BlockBreakEvent $ev){
    ...
    if (isset($this->cases[$lv->getName()][$cid])) {
      if (!Perms::access($pl,"itemcase.destroy")) {
	$ev->setCancelled();
	return;
      }
      $pl->sendMessage(mc::_("Destroying ItemCase %1%",$cid));
